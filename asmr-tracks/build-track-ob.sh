...
echo "=== QE Obedience Sessions Builder v1.0 — Track ${TRACK} ==="
echo ""
...
# -----------------------------------------------------------
echo "[3/5] Generating singing bowl gradient (warm, under voice)..."
BOWL_SRC="$SCRIPT_DIR/singing-bowl-warm-v1.mp3"
...
for level in 22 18 18 14 14 12; do
    vol=$(python3 -c "print(round(-${level}/100 * abs(float('${VOICE_LOUD}')) + 18, 1))")
    ffmpeg -y -i "$BOWL_SRC" -af "volume=${vol}dB" -b:a 192k "$WORK/bowl-${level}.mp3" 2>&1 | tail -1
done
echo ""
...
case "$TRACK" in
    01)
        ...
        for i in $(seq 2 $NUM_SEGMENTS); do
            idx=$(( (i-1) % 6 ))
            echo "file 'silence-2s.mp3'" >> "$CONCAT"
            echo "file 'bowl-${BOWL_LEVELS[$idx]}.mp3'" >> "$CONCAT"
            echo "file 'silence-3s.mp3'" >> "$CONCAT"
            echo "file 'voice-proc-${i}.mp3'" >> "$CONCAT"
            echo "file 'silence-8s.mp3'" >> "$CONCAT"
        done
        echo "file 'silence-8s.mp3'" >> "$CONCAT"
        echo "file 'bowl-12.mp3'" >> "$CONCAT"
